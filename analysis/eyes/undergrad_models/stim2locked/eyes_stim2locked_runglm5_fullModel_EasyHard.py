...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb 12 14:25:01 2024
...
trlcheckdir = op.join(wd, 'data', 'eyes', 'trialchecks', 'stim2locked')
...
gave        = np.zeros(shape = [subs.size, 4201])
...
for i in subs:
    ...
    trlnans_perc = np.load(file = op.join(trlcheckdir, f"{param['subid']}_stim2locked_eyes_nancheck_perTrial.npy"))
    ...
    np.save(file = op.join(glmdir, param['subid'] + '_stim2lockedEyes_betas.npy'), arr = betas)
    np.save(file = op.join(glmdir, param['subid'] + '_stim2lockedEyes_copes.npy'), arr = copes)
    np.save(file = op.join(glmdir, param['subid'] + '_stim2lockedEyes_tstats.npy'), arr = tstats)
