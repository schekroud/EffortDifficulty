...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb  2 19:30:40 2024
...
for i in subs:
    ...
    tmpdiff2[0] = np.nanmean(np.squeeze(stim1locked['rewarded==1 and difficultyOri == 2'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)
    tmpdiff2[1] = np.nanmean(np.squeeze(stim1locked['unrewarded==1 and difficultyOri == 2'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)    
    diff2[icount] = tmpdiff2
    
    tmpdiff4 = np.zeros(shape=[2,times.size]) #correct and incorrect, 7001 time points
    tmpdiff4[0] = np.nanmean(np.squeeze(stim1locked['rewarded==1 and difficultyOri == 4'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)
    tmpdiff4[1] = np.nanmean(np.squeeze(stim1locked['unrewarded==1 and difficultyOri == 4'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)    
    diff4[icount] = tmpdiff4
    
    tmpdiff8 = np.zeros(shape=[2,times.size]) #correct and incorrect, 7001 time points
    tmpdiff8[0] = np.nanmean(np.squeeze(stim1locked['rewarded==1 and difficultyOri == 8'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)
    tmpdiff8[1] = np.nanmean(np.squeeze(stim1locked['unrewarded==1 and difficultyOri == 8'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)    
    diff8[icount] = tmpdiff8
    
    tmpdiff12 = np.zeros(shape=[2,times.size]) #correct and incorrect, 7001 time points
    tmpdiff12[0] = np.nanmean(np.squeeze(stim1locked['rewarded==1 and difficultyOri == 12'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)
    tmpdiff12[1] = np.nanmean(np.squeeze(stim1locked['unrewarded==1 and difficultyOri == 12'].pick(picks='pupil_transformed').apply_baseline(baseline).get_data()), axis=0)    
...
fig.savefig(op.join(figdir, 'stim1lockedEyes_DifficultyXcorrectness.pdf'), dpi = 300, format = 'pdf')
fig.savefig(op.join(figdir, 'stim1lockedEyes_DifficultyXcorrectness.png'), dpi = 300, format = 'png')
