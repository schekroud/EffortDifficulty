...
# -*- coding: utf-8 -*-
"""
Created on Mon Feb  5 23:15:10 2024
...
if op.exists(fname) and check_exists:
    ...
    for i in subs:
        ...
        pupil_data = np.squeeze(stim1locked.pick(picks='pupil_transformed')._data)
        ...
        for tp in range(times.size): #loop over time points
            ...
            #ISI = sp.stats.zscore(ISI) #zscore this too
            ...
            intercept = np.ones_like(trlnum, dtype = int)
            
            desmat = np.array([intercept, acc, difficulty, prevtrlfb]).T#, ISI]).T
            dm = pd.DataFrame(desmat, columns = ['intercept', 'accuracy', 'difficulty', 'prevtrlfb'])#, 'ISI'])
...
for isub in range(subs.size):
    ...
    intercept[isub]  = tmpdf.intercept.to_numpy()
...
# ax.plot(times, isi_mean, label = 'ISI', color = cols[4])
# ax.fill_between(times, np.add(isi_mean, isi_sem), np.subtract(isi_mean, isi_sem),
#                 color = cols[4], alpha = 0.3, lw=0)
ax.legend(loc = 'right', bbox_to_anchor = [1.2, 0.75])
