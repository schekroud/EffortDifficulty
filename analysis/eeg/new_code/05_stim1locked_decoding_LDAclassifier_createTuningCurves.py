# -*- coding: utf-8 -*-
"""
Created on Thu Apr 18 10:48:16 2024
...
for i in subs:
    ...
    for tp in range(ntimes): #for each time point
        preds, predprobas = run_decoding_predproba(data_pos, labelbins, tp, use_pca, testtype, classifier='LDA') #run decoding
        #this outputs:
    ...
    plot = ax.imshow(centred_probabilities.mean(0), aspect='auto', interpolation = 'none',
              cmap = 'RdBu_r', extent = [epochs.times.min(), epochs.times.max(), 0, 6])#, vmin = -0.1, vmax = 0.4)
#%%
...
plt.figure(); plt.plot(range(theta2.size), theta2)

#just look at raw accuracy now
