# -*- coding: utf-8 -*-
"""
Created on Tue Mar 26 16:02:05 2024
...
for i in subs:
    ...
    if i not in [29, 30]:
        ...
        epoched = mne.Epochs(raw, events, events_s1, tmin, tmax, baseline, reject_by_annotation=False, preload=True)
    ...
    np.savetxt(fname = op.join(param['path'], 'removed_comps', 's%02d_removedcomps_stim1locked.txt'%i),
    ...
    epoched.save(fname = param['stim1locked'].replace('stim1locked', 'stim1locked_icaepoched'), fmt = 'double', overwrite = True)
    ...
    del(eog_epochs)
        ...
        epoched1 = mne.Epochs(raw1, events1, events_s1, tmin, tmax, baseline, reject_by_annotation=False, preload=True)
        epoched2 = mne.Epochs(raw2, events2, events_s1, tmin, tmax, baseline, reject_by_annotation=False, preload=True)
        ...
        comps2rem = input('components  remove: ') #need to separate component numbers by comma and space
        comps2rem = list(map(int, comps2rem.split(', ')))
        np.savetxt(fname = op.join(param['path'], 'removed_comps', 's%02db_removedcomps_stim1locked.txt'%i),
                   X = comps2rem, fmt = '%i') #record what components were removed
...
for i in subs:
    ...
    epoched = mne.read_epochs(fname = param['stim1locked'].replace('stim1locked', 'stim1locked_icaepoched'), preload =True)
    epoched.resample(500) #downsample to 500hz to save space
    epoched.save(fname = param['stim1locked'].replace('stim1locked', 'stim1locked_icaepoched_resamp'), fmt = 'double', overwrite = True)
