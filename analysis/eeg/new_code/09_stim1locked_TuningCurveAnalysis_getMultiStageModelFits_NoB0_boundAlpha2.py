...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul  1 20:36:16 2024
...
for i in subs:
    ...
    for smooth_alphas in [True, False]:
        ...
        if not op.exists(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'fixedalpha_b1only_2alpha', f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}_smoothedAlpha_{smooth_alphas}{addSmooth}_no_bzero.npy')):
            ...
            for trl in range(ntrials):   #loop over trials
                for tp in range(ntimes): #and loop over time points
                    ...
                    tpa = TuningCurveFuncs.fitAlpha(binmidsrad, dztp, bounds = ([0], [2])) #bound alpha between 0 and 2
                    alphas[trl,tp] = tpa[0]
            ...
            np.save(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'fixedalpha_b1only_2alpha', f's{i}_ParamFits_Alpha_binstep{binstep}_binwidth{binwidth}_smoothedAlpha_{smooth_alphas}{smooth_sigma}_no_bzero.npy'), arr = alphas)
            np.save(op.join(wd, 'data', 'tuningcurves', 'parameter_fits', 'fixedalpha_b1only_2alpha', f's{i}_ParamFits_Betas_binstep{binstep}_binwidth{binwidth}_smoothedAlpha_{smooth_alphas}{smooth_sigma}_no_bzero.npy'), arr = betas)