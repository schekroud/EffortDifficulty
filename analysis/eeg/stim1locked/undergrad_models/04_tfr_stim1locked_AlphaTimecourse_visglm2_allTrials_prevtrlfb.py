...
for iglm in [0, 1]:
    ...
    for i in subs:
        ...
        ibetas  = np.load(file = op.join(glmdir, param['subid'] + f'_stim1lockedTFR_betas{addtopath}.npy'))
        icopes  = np.load(file = op.join(glmdir, param['subid'] + f'_stim1lockedTFR_copes{addtopath}.npy'))
        itstats = np.load(file = op.join(glmdir, param['subid'] + f'_stim1lockedTFR_tstats{addtopath}.npy'))
    ...
    ax.set_ylabel('Alpha power (Beta, AU)')
    fig.tight_layout()
    fig.savefig(op.join(figpath, f'model1_allTrials_prevtrlfb_prevtrlCorrectIncorrect{addtopath}.pdf'), format = 'pdf', dpi = 400)
