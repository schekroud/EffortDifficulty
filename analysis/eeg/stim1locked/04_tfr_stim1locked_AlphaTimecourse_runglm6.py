# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 23:06:28 2023
...
glmdir = op.join(wd, 'glms', 'stim1locked', 'alpha_timecourses', 'glm6')
...
for i in subs:
    for iglm in [1]: #controls whether glm is run on un-baselined or baselined data
        ...
        tfr = tfr['diffseqpos <= 12'] #only keep trials 1-12 in the difficulty sequence
        ...
        diffseqpos = tfr.metadata.diffseqpos.to_numpy()
        diffseqposz = sp.stats.zscore(diffseqpos, nan_policy='omit')
        ...
        # for diffpos in sorted(tfr.metadata.diffseqpos.unique()):
        #     DC.add_regressor(name = 'diffseqpos'+str(diffpos), rtype = 'Categorical', datainfo = 'diffseqpos', codes = diffpos)
        DC.add_regressor(name = 'diffseqpos', rtype = 'Parametric', datainfo = 'diffseqpos', preproc = None)
