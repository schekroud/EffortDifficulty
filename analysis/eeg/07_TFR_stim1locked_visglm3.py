# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 15:48:35 2023
...
regs = ['diff2', 'diff4', 'diff8', 'diff12', 'correctness']
contrasts = ['diff2', 'diff4', 'diff8', 'diff12', 'correctness', 'grandmean']
...
for i in subs:
    ...
    for name in regs:
        b[name].append(
            mne.time_frequency.read_tfrs(  fname = op.join(wd, 'glms', 'stim1locked', 'glm3', 
                                          param['subid'] + '_stim1locked_tfr_' + name + addtopath + '_beta-tfr.h5'))[0].crop(tmin=-2.85, tmax=2.85))
            
    for name in contrasts:
        c[name].append(mne.time_frequency.read_tfrs(fname = op.join(wd, 'glms', 'stim1locked', 'glm3', 
                                      param['subid'] + '_stim1locked_tfr_' + name + addtopath + '_cope-tfr.h5'))[0].crop(tmin=-2.85, tmax=2.85))
        t[name].append(mne.time_frequency.read_tfrs(fname = op.join(wd, 'glms', 'stim1locked', 'glm3', 
                                      param['subid'] + '_stim1locked_tfr_' + name + addtopath + '_tstat-tfr.h5'))[0].crop(tmin=-2.85, tmax=2.85))

#%%
alltimes = mne.grand_average(b['diff2']).times
allfreqs = mne.grand_average(b['diff2']).freqs
#%%
timefreqs       = {(.4, 10):(.4, 4),
                   (.6, 10):(.4, 4),
                   (.8, 10):(.4, 4),
...
for idiff in range(len(diffs)):
    tmpdat = tmp['diff'+str(difficulties[idiff])] #list of averageTFRs per subject
