...
for i in subs:
    ...
    if i not in [29, 30]:
        ...
        events, _ = mne.events_from_annotations(raw, event_id)
        epoched = mne.Epochs(raw, events, events_s1, tmin, tmax, baseline,
        ...
        epoched2 = mne.Epochs(raw2, events2, events_s1, tmin, tmax, baseline,
                             reject_by_annotation=False, preload=False, on_missing='ignore') #it doesn't like to continue if it doesn't find a trigger
        #need to cut down 
        ...
        epochs.save(fname = param['stim1locked'], overwrite = True)
