...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun  7 11:43:51 2023
...
for i in subs:
    ...
    epochs = mne.read_epochs(fname = param['stim1locked'].replace('stim1locked', 'stim1locked_cleaned'), preload = True)
...
gave_diff.plot_joint(times = np.arange(0, 0.6, 0.1), topomap_args = topoargs)
gave_corr.plot_joint(times = np.arange(0, 0.6, 0.1), topomap_args = topoargs)
gave_incorr.plot_joint(times = np.arange(0, 0.6, 0.1), topomap_args = topoargs)