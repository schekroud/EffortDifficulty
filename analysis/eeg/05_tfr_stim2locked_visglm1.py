...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 16 11:48:33 2023
...
betas     = ['grandmean', 'correctness']
contrasts = ['grandmean', 'correct', 'incorrect', 'correctness']
...
for i in subs:
    ...
    for name in betas:
        b[name].append(
            mne.time_frequency.read_tfrs(  fname = op.join(wd, 'glms', 'stim2locked', 'glm1', 
                                          param['subid'] + '_stim2locked_tfr_AlphaOnly_' + name + '_beta-tfr.h5'))[0])
            
    for name in contrasts:
        c[name].append(mne.time_frequency.read_tfrs(fname = op.join(wd, 'glms', 'stim2locked', 'glm1', 
                                      param['subid'] + '_stim2locked_tfr_AlphaOnly_' + name + '_cope-tfr.h5'))[0])
        t[name].append(mne.time_frequency.read_tfrs(fname = op.join(wd, 'glms', 'stim2locked', 'glm1', 
                                      param['subid'] + '_stim2locked_tfr_AlphaOnly_' + name + '_tstat-tfr.h5'))[0])
...
#%%
alltimes = mne.grand_average(b['grandmean']).times
allfreqs = mne.grand_average(b['grandmean']).freqs
#%%
timefreqs       = {(.4, 10):(.4, 4),
                   (.6, 10):(.4, 4),
                   (.8, 10):(.4, 4),
